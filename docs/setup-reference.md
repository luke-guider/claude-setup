...
| `mempalace/config.example.json` | Template for `~/.mempalace/config.json` (wings, palace path) | **Committed** (reference) |
| `mempalace/wing_config.example.json` | Template for `~/.mempalace/wing_config.json` (wing keywords) | **Committed** (reference) |
| `mempalace/identity.example.txt` | Template for `~/.mempalace/identity.txt` (agent persona) | **Committed** (reference) |
| `mempalace/mine/mine-workspace.example.sh` | Template for per-wing mining scripts | **Committed** (reference) |
| `~/.mempalace/{config,wing_config}.json`, `identity.txt`, `mine-*.sh` | Live per-user config copied from templates at install | Excluded |
